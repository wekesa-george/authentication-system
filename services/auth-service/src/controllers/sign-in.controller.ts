...
import {LoginType, OtpService} from '@sourceloop/authentication-service';
import {
...
import {
  ActorId, AuthClient, AuthClientRepository, AuthCodeBindings,
  AuthCodeGeneratorFn, AuthRefreshTokenRequest, AuthServiceBindings, AuthTokenRequest,
  AuthUser, CodeReaderFn, CodeResponse, ExternalTokens,
  IUserActivity, JWTSignerFn,
  JWTVerifierFn,
  JwtPayloadFn, LoginActivity, LoginActivityRepository, LoginHelperService, LoginRequest, OtpCacheRepository, RefreshToken, RefreshTokenRepository, ResetPassword, RevokedTokenRepository,
  RoleRepository,
  TenantConfigRepository, TokenResponse, User, UserCredentialsRepository,
  UserLevelPermissionRepository,
  UserLevelResourceRepository, UserTenant, UserTenantRepository, UserValidationFn, UserValidationServiceBindings
...
export class LoginController {
  constructor(
    ...
    public userRepo: UserRepositoryNew,
...
*/
      ...
      return this.createJWT(payload, authClient, LoginType.ACCESS);
    } catch (error) {
      this.logger.error(error);
  ...
  @authorize({permissions: ['*']})
  @patch(`auth/change-password-new`, {
    security: OPERATION_SECURITY_SPEC,
    responses: {
      [STATUS_CODE.OK]: {
