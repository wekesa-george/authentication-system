...
import {
  AuthClientRepository, AuthCodeBindings, AuthServiceBindings, AuthTokenRequest,
  AuthUser, CodeReaderFn, CodeResponse, CodeWriterFn, IMfaConfig, IOtpConfig, JWTVerifierFn,
  MfaCheckFn, OtpCacheRepository, OtpLoginRequest, OtpMethodType, QrCodeCheckResponse,
  QrCodeCreateResponse, User, UserCredentials, UserCredentialsRepository,
  UserRepository, VerifyBindings
...
export class OtpControllerNew {
  ...
  async sendOtp(
    ...
    // This is intentional
    const user = await this.userRepo.findById(req.user_id);
    if (!user) {
    ...
    try {
      ...
      return this.otpService.sendOtp(user, client)
