<script lang="ts">
  ...
  let isValid= true;
	...
	export let selectedCountry: CountryCode | null = 'UG';
	...
	$: if($formData[item.name+"_detailed"]){
		$formData[item.name] = $formData[item.name+"_detailed"].phoneNumber
		fullNumber = $formData[item.name+"_detailed"].phoneNumber
...
>
	...
	<TelInput
		...
		bind:detailedValue={$formData[item.name+"_detailed"]}
		value={fullNumber}
		bind:valid
		name="{item.name}"
		{options}
